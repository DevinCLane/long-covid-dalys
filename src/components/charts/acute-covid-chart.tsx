 ...
 */
function acuteDalysByYear(
  scenarios: typeof chartData.scenarios,
): DalysDataItem[] {
  const byYear = new Map<number, DalysDataItem>();

  for (const scenario of scenarios) {
    const acute = scenario.conditions.find(
      (condition) => condition.condition === "acute_covid",
    );
    if (!acute?.years.length) continue;

    for (const yearPoint of acute.years) {
      const row =
...
const chartDataItems = acuteDalysByYear(chartData.scenarios);
      ...
      <CardHeader className="flex items-center gap-2 space-y-0 border-b sm:flex-row">
        <div className="grid flex-1 gap-1 text-center sm:text-left">
          <CardTitle className="text-l text-pretty md:text-2xl">
            5-year DALYs for Acute COVID-19
          </CardTitle>
        </div>
      </CardHeader>
      <CardContent>
        <div className="flex flex-col">
          ...
          <div
            ref={setLegendPortal}
            data-chart="chart-acute-covid-dalys"
